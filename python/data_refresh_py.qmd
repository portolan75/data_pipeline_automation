...
for i in m.index:
    ...
    if meta_temp["success"]:
        ...
        meta_temp["comments"] = meta_temp["comments"] + "The data refresh done anyway, please check the log; "
        print("Data Refresh done anyway -  to avoid blocking further updates")
        # IN THE FUTURE ADD FUNCTION TO RUN EVERY 6 MONTHS TO CHECK FOR UPDATES FOR PAST SEMESTER AND EVENTUALLY REFRESH DATA
        d = eia_data.append_data(data_path = data_path, new_data = ts_obj, save = True)
        meta_temp["update"] = True
...
if data is not None:
    ...
    print("No new data is available since last refresh -> Check GitHub Actions previous Data Refresh workflow run")
```
...
full_data = full_data.sort_values(by = ["subba", "period"])
full_data.head()

full_data["period"] = pd.to_datetime(full_data["period"])
fig = px.line(full_data, x="period", y="value", color="subba")
fig.show()
# fig
```